...
const MarksCard = ({ name, marks, code, category, overall }: Props) => {
  ...
  useEffect(() => {
    setReq(
      (((grade_points[grade] - Number(overall.marks)) / 40) * 75).toPrecision(3)
    );
  }, [grade, expanded, overall]);
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className={styles.markPill} title={`Lost ${(parseFloat(overall.total) - parseFloat(overall.marks)).toPrecision(3)} marks`}>
          ...
          <span id="warn">This shows how many marks you want to get in the final semester exam! So make sure ur total internal is at 60</span>
          </div>
        </div>
      ) : null}
    </>
