...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      classNames={{
        months: "flex flex-col w-fit mx-auto space-y-4 sm:space-x-4 sm:space-y-0",
        month_caption: "flex justify-start pb-2 w-fit relative items-center px-3",
      ...
      components={{
          Chevron: (props) => {
            if(props.orientation === "left") {
              return <LuChevronLeft className={`h-4 w-4 ${props.disabled ? "opacity-30 cursor-not-allowed" : ""}`} />;
            } else {
              return <LuChevronRight className={`h-4 w-4 ${props.disabled ? "opacity-30 cursor-not-allowed" : ""}`} />;
            }
          }
      }}
      {...props}
