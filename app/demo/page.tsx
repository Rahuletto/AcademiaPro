    ...
    const marks: Mark[] = json.courses.courses.map(c => {
        ...
        const testPerformance = Array.from({ length: testCount }, (_, i) => {
            ...
            return {
                test: `${testType}-${i + 1}`,
                marks: {
                    scored: (Math.random() * (total * 0.8 - total * 0.5) + total * 0.5).toFixed(2),
                    total: total.toFixed(2)
                }
            };
        });

        const totalScored = testPerformance.reduce((sum, test) => 
            sum + Number.parseFloat(test.marks.scored), 0
        );
        const totalPossible = testPerformance.reduce((sum, test) => 
            sum + Number.parseFloat(test.marks.total), 0
        );
  ...
  return (
    <div className="h-screen flex-shrink-0 w-full flex flex-row bg-light-background-normal lg:p-2 transition-all duration-150 text-light-color dark:bg-dark-background-normal dark:text-dark-color">
			...
			>
				...
				>
					<div style={{ height: "-webkit-fill-available" }}><div id="items" className="flex flex-col gap-12 relative">
				...
				<Suspense fallback={<Loading size="xl" />}>
					<Marks marks={marks} courses={json.courses?.courses as Course[]} />
				</Suspense>
			</div></div>
				</main>
			</div>
